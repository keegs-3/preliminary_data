...
def get_substance_score(patient_answers):
    ...
    for sub, qmap in SUBSTANCE_QUESTIONS.items():
        ...
        if is_current:
            ...
            score = score_substance_use(use_band, years_band, False)
...
# Ensure pillar_map and PILLARS are defined before this code:
pillar_map = {
...
# --- Main Loop: Calculate scores per patient ---
all_scores = []
...
for idx, row in patient_survey.iterrows():
    ...
    for qid, config in QUESTION_CONFIG.items():
        ...
        # Standard map-based scoring
        else:
            score = config.get("response_scores", {}).get(str(answer).strip(), 0)

        # Store weighted and raw scores per pillar
        for pillar, wt in config.get("pillar_weights", {}).items():
            if wt and wt > 0:
                patient_result[f"{qid}_{pillar}_weighted"] = score * wt
                patient_result[f"{qid}_{pillar}_raw"] = score

    # Custom scoring for Movement questions
    ...
    # Custom scoring for Sleep issues
    ...
    # Sleep hygiene protocols scoring (4.07)
    sleep_proto_score = score_sleep_protocols(row.get("4.07", ""))
    if sleep_proto_score and sleep_proto_score > 0:
...
# Create DataFrame from all patient scores and fill NAs with zero
df_debug = pd.DataFrame(all_scores).fillna(0)

# Aggregate weighted scores by pillar for each patient
for pillar in PILLARS:
    weighted_cols = [col for col in df_debug.columns if col.endswith(f"_{pillar}_weighted")]
    if weighted_cols:
        df_debug[pillar_map[pillar]] = df_debug[weighted_cols].sum(axis=1)
    else:
        df_debug[pillar_map[pillar]] = 0

# --- Calculate max possible weighted score per pillar ---

max_scores_per_pillar = {pillar: 0 for pillar in PILLARS}

# Sum max possible scores from QUESTION_CONFIG
for qid, config in QUESTION_CONFIG.items():
    for pillar, wt in config.get("pillar_weights", {}).items():
        if wt and wt > 0:
            max_resp = 0
            if "response_scores" in config and config["response_scores"]:
                ...
                max_resp = 10  # Adjust this if your scoring functions have different max
            max_scores_per_pillar[pillar] += max_resp * wt

# Add max scores from movement questions
for cfg in movement_questions.values():
    for pillar, wt in cfg.get("pillar_weights", {}).items():
        max_scores_per_pillar[pillar] += wt  # full weight per movement type

# Add max from sleep issues
sleep_issue_max = sum(pillar_wts.get("Sleep", 0) for _, _, pillar_wts in SLEEP_ISSUES)
max_scores_per_pillar["Sleep"] += sleep_issue_max

# Add sleep hygiene protocols fixed weight
max_scores_per_pillar["Sleep"] += 9.0

# Add stress max weights (stress level + coping skills)
max_scores_per_pillar["Stress"] += 19 + 7

# Add substance use max weights to CoreCare
max_scores_per_pillar["CoreCare"] += sum(SUBSTANCE_WEIGHTS.values())

# Append max and percentage columns for each pillar to df_debug
for pillar in PILLARS:
    max_val = max_scores_per_pillar.get(pillar, 1)  # Avoid division by zero
    col_name = pillar_map[pillar]
    df_debug[f"{col_name}_Max"] = max_val
    df_debug[f"{col_name}_Pct"] = (df_debug[col_name] / max_val) * 100

# Add missing substance columns if absent (optional)
substance_cols = [
    "Substance: Tobacco",
    "Substance: Alcohol",
...
final_cols = (
    ...
    [f"{pillar_map[p]}_Pct" for p in PILLARS] +
    substance_cols
)
