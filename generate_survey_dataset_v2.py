...
# UPDATED: SUBSTANCE_META with new time since quit question IDs
SUBSTANCE_META = [
    {
        "name": "Tobacco (cigarettes, cigars, smokeless tobacco)",
        "current": {"freq": "8.02", "years": "8.03", "pattern": "8.04"},
        "past": {"years": "8.21", "freq": "8.22", "time_since_quit": "8.23"}  # NEW
    },
    {
        "name": "Alcohol",
        "current": {"freq": "8.05", "years": "8.06", "pattern": "8.07"},
        "past": {"years": "8.24", "freq": "8.25", "time_since_quit": "8.26"}  # NEW (renumbered)
    },
    {
        "name": "Recreational drugs (e.g., marijuana)",
        "current": {"freq": "8.08", "years": "8.09", "pattern": "8.10"},
        "past": {"years": "8.27", "freq": "8.28", "time_since_quit": "8.29"}  # NEW (renumbered)
    },
    {
        "name": "Nicotine",
        "current": {"freq": "8.11", "years": "8.12", "pattern": "8.13"},
        "past": {"years": "8.30", "freq": "8.31", "time_since_quit": "8.32"}  # NEW (renumbered)
    },
    {
        "name": "Over-the-counter medications (e.g., sleep aids)",
        "current": {"freq": "8.14", "years": "8.15", "pattern": "8.16"},
        "past": {"years": "8.33", "freq": "8.34", "time_since_quit": "8.35"}  # NEW (renumbered)
    },
    {
        "name": "Other",
        "current": {"freq": "8.17", "years": "8.18", "pattern": "8.19"},
        "past": {"years": "8.36", "freq": "8.37", "time_since_quit": "8.38"}  # NEW (renumbered)
    }
]

# UPDATED: Random substance answer function with time_since_quit support
def random_substance_answer(subst, qtype):
    ...
    elif qtype == "time_since_quit":  # NEW
        return random.choice(SUBSTANCE_QS[subst]["time_since_quit"])
    return ""
...
# UPDATED: Past use function with time since quit data
def past_use_rowdata(substances):
    ...
    for meta in SUBSTANCE_META:
        ...
        # NEW: Add time since quit data for former users
        rowdata[tags["time_since_quit"]] = random.choice(SUBSTANCE_QS[meta["name"]]["time_since_quit"])
    return rowdata

# --- Generate all substance use rowdata ---

def answer_8_39(patient, substances, past_substances):
...
def answer_8_40(patient, substances):
...
def answer_8_41(patient, substances):
...
def answer_8_42(patient, substances):
...
def answer_8_43(patient, substances):
...
def answer_8_45(patient, takes_dietary_supps):
...
def answer_8_46(patient, takes_dietary_supps):
...
def answer_8_48(patient, takes_performance_supps):
...
def answer_8_49(patient, takes_performance_supps):
...
def answer_8_52(patient, sleep_aid_freq):
...
def answer_8_53(patient, aid_types):
...
def answer_8_54(patient, sleep_aid_freq, aid_types):
...
def answer_8_57(patient, takes_more):
...
def answer_8_59(patient, floss_freq):
...
def answer_8_61(patient, brush_freq):
...
def answer_8_63(patient, sunscreen_freq):
...
def answer_8_64(patient):
    """Consistent skincare routine?"""
    return random.choices(["Yes", "No"], weights=[0.45, 0.55])[0]

def answer_8_65(patient, skincare):
...
def generate_survey_responses(profile_csv, out_csv=None, seed=42):
    ...
    for _, row in df_profiles.iterrows():
        ...
        takes_dietary_supps = answer_8_44(patient)
        dietary_supps_list = answer_8_45(patient, takes_dietary_supps)
        consider_dietary_supps = answer_8_46(patient, takes_dietary_supps)
        takes_performance_supps = answer_8_47(patient)
        performance_supps_list = answer_8_48(patient, takes_performance_supps)
        consider_performance_supps = answer_8_49(patient, takes_performance_supps)
        takes_sleep_supps = answer_8_50(patient)
        sleep_aid_freq = answer_8_51(patient, takes_sleep_supps)
        sleep_aid_types = answer_8_52(patient, sleep_aid_freq)
        sleep_nat_supps = answer_8_53(patient, sleep_aid_types)
        consider_sleep_supps = answer_8_54(patient, sleep_aid_freq, sleep_aid_types)
        takes_more = answer_8_55(patient)
        more_supps_list = answer_8_56(patient, takes_more)
        consider_more = answer_8_57(patient, takes_more)
        ...
        row_data = {
            ...
            **substance_rowdata,
            '8.39': "|".join(answer_8_39(patient, substances, past_substances)),
            '8.40': "|".join(answer_8_40(patient, substances)),
            '8.41': answer_8_41(patient, substances),
            '8.42': "|".join(answer_8_42(patient, substances)),
            '8.43': answer_8_43(patient, substances),
            '8.44': takes_dietary_supps,
            '8.45': dietary_supps_list,
            '8.46': consider_dietary_supps,
            '8.47': takes_performance_supps,
            '8.48': performance_supps_list,
            '8.49': consider_performance_supps,
            '8.50': takes_sleep_supps,
            '8.51': sleep_aid_freq,
            '8.52': "|".join(sleep_aid_types),
            '8.53': "|".join(sleep_nat_supps),
            '8.54': consider_sleep_supps,
            '8.55': takes_more,
            '8.56': more_supps_list,
            '8.57': consider_more,
            '8.58': floss_freq,
            '8.59': answer_8_56(patient, floss_freq),
            '8.60': brush_freq,
            '8.61': answer_8_61(patient, brush_freq),
            '8.62': sunscreen_freq,
            '8.63': answer_8_63(patient, sunscreen_freq),
            '8.64': skincare,
            '8.65': answer_8_65(patient, skincare),
